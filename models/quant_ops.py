...
def round_width(width, wm=None,
                width_divisor=getattr(OPTION, 'width_divisor', 1),
                min_width=getattr(OPTION, 'min_width', 1)):
    if not wm:
        return width
    width *= wm
...
class SwitchBN2d(nn.Module):
    ...
    def forward(self, input):
        if getattr(OPTION, 'switchbn', True):
            if self.bits in OPTION.bits_list:
                idx = OPTION.bits_list.index(self.bits)
            else:
                idx = 0
            y = self.bn[idx](input)
            if not self.affine:
...
class QConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels,
        ...
        self.weight_only = weight_only or getattr(OPTION, 'weight_only', False)
        self.quant = q_k.apply
        if getattr(OPTION, 'switch_alpha', False):
            self.alpha = nn.Parameter(torch.ones(len(OPTION.bits_list)) * 8.0)
        else:
    ...
    def forward(self, input):
        ...
        weight_quant_scheme = getattr(OPTION, 'weight_quant_scheme', 'modified')
        act_quant_scheme = getattr(OPTION, 'act_quant_scheme', 'original')
        ...
        if bitw < 32:
            ...
            weight.mul_(2.0)
            weight.sub_(1.0)
            if getattr(OPTION, 'rescale_conv', False):
                rescale_type = getattr(OPTION, 'rescale_type', 'constant')
        ...
        if (bita < 32 and not self.weight_only) or self.pact_fp:
            if getattr(OPTION, 'switch_alpha', False):
                if bita in OPTION.bits_list:
                    idx_alpha = OPTION.bits_list.index(bita)
                else:
                    idx_alpha = 0
...
class QLinear(nn.Linear):
    def __init__(self, in_features, out_features, bias=True, bitw_min=None, bita_min=None, pact_fp=False, weight_only=False):
        ...
        self.quant = q_k.apply
        if getattr(OPTION, 'switch_alpha', False):
            self.alpha = nn.Parameter(torch.ones(len(OPTION.bits_list)) * 10.0)
        else:
    ...
    def forward(self, input):
        ...
        if bitw < 32:
            ...
            weight.mul_(2.0)
            weight.sub_(1.0)
            if getattr(OPTION, 'rescale', True):
                rescale_type = getattr(OPTION, 'rescale_type', 'constant')
                if rescale_type == 'stddev':
                    weight_scale = torch.std(self.weight.detach())
                elif rescale_type == 'constant':
            ...
            if getattr(OPTION, 'rescale', True):
                ...
                if self.training:
                   weight.mul_(weight_scale)
        bias = self.bias
        if bias is not None and getattr(OPTION, 'rescale', True) and not self.training:
