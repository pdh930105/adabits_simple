...
class ResNet_CIFAR(nn.Module):
    def __init__(self, num_classes=10):
        ...
        AssertionError(OPTION==None, "please insert config option")
        # head
        channels = 16
        l_head = QConv2d(3, channels, 3, 1, 1, bias=False,
                         bitw_min=max(OPTION.bits_list), bita_min=8, weight_only=True)
        ...
        self.block_setting = self.block_setting_dict[OPTION.depth]
        ...
        self.classifier = nn.Sequential(
            QLinear(
                outp,
                num_classes,
                bitw_min=max(OPTION.bits_list)
            )
        )

        if getattr(OPTION, "reset_parameters", False):
...
def resnet_cifar(num_classes=100):
