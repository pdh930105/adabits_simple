...
class InvertedResidual(nn.Module):
    def __init__(self, inp, outp, stride, expand_ratio, double_side=False):
        ...
        if expand_ratio != 1:
            layers += [
                QConv2d(inp, expand_inp, 1, 1, 0, bias=False, pact_fp=getattr(OPTION, 'pact_fp', False), double_side=double_side),
                SwitchBN2d(expand_inp, affine=not getattr(OPTION, 'stats_sharing', False)),
        ...
        layers += [
            QConv2d(
                ...
                pact_fp=getattr(OPTION, 'pact_fp', False),
                double_side=double_side if expand_ratio == 1 else False),
            SwitchBN2d(expand_inp, affine=not getattr(OPTION, 'stats_sharing', False)),
            nn.ReLU(inplace=True),
            QConv2d(expand_inp, outp, 1, 1, 0, bias=False, pact_fp=getattr(OPTION, 'pact_fp', False)),
            SwitchBN2d(outp, affine=not getattr(OPTION, 'stats_sharing', False)),
        ]
    ...
    def __init__(self, num_classes=1000):
        ...
        self.head = nn.Sequential(
                        QConv2d(
                            ...
                            weight_only=True),
                        SwitchBN2d(channels, affine=not getattr(OPTION, 'stats_sharing', False)),
                        nn.ReLU(inplace=True),
                    )

        double_side = getattr(OPTION, 'double_side', False)
        # body
